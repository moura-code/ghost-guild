...
Expected cumulative test count: T6 285 · T7 294 · T8 306 · T9 314 · T10 321 · T11 330.
...
Then the whole tree: `cmd //c "tools\test.cmd tests"` → **285 cases**, exit 0.
...
Run: `cmd //c "tools\test.cmd tests/game/guild_screen_test.gd"`
Expected: PASS, 8 cases. Then `cmd //c "tools\test.cmd tests"` → **294 cases**, exit 0.
...
Run: `cmd //c "tools\test.cmd tests/game/seance_screen_test.gd"`
Expected: PASS, 12 cases. Then `cmd //c "tools\test.cmd tests"` → **306 cases**, exit 0.
...
- [ ] **Step 5: Check the stat name strings exist**

`_build()` reads `stat.might.name` … `stat.focus.name`. Run:

```bash
grep -n "^stat\." data/strings/en.csv
```

If any of the four is missing, append it (`stat.might.name,Might`, `stat.wit.name,Wit`, `stat.vigor.name,Vigor`, `stat.focus.name,Focus`) as part of this task's Step 1 and say so in your report.

- [ ] **Step 6: Run the tests to verify they pass**

Run: `cmd //c "tools\test.cmd tests/game/hero_screen_test.gd"`
Expected: PASS, 8 cases. Then `cmd //c "tools\test.cmd tests"` → **314 cases**, exit 0.
...
Run: `cmd //c "tools\test.cmd tests/game/offline_summary_test.gd"`
Expected: PASS, 7 cases. Then `cmd //c "tools\test.cmd tests"` → **321 cases**, exit 0.
...
Run: `cmd //c "tools\test.cmd tests/game/main_test.gd"`
Expected: PASS, 9 cases. Then the whole tree: `cmd //c "tools\test.cmd tests"` → **330 cases**, exit 0.
